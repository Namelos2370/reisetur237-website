...
const renderDashboard=()=>(
      ...
      <div style={{background:'#fff',borderRadius:14,padding:isMobile?'16px':'20px 24px',boxShadow:'0 2px 12px rgba(0,0,0,0.06)',border:'1px solid #F1F5FB',marginBottom:16}}>
        ...
        {visa?VISA_STEPS.map((step,i)=>{
          ...
          return(<div key={step.key} style={{display:'flex',alignItems:'center',gap:12,marginBottom:10}}>
            <div style={{width:26,height:26,borderRadius:'50%',flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',background:done?'#10B981':curr?RED:'#E2E8F0',color:done||curr?'#fff':'#94A3B8',fontSize:11,fontWeight:700}}>{done?'✓':i+1}</div>
  ...
  const renderDocuments=()=>(
    <div>
      ...
      {docs.length===0?<div style={{background:'#fff',borderRadius:14,padding:'40px',textAlign:'center',color:'#94A3B8',boxShadow:'0 2px 12px rgba(0,0,0,0.06)'}}><FileText size={32} style={{marginBottom:10,opacity:.3}}/><p>Aucun document déposé.</p></div>
        :docs.map(doc=>(
          <div key={doc.id} style={{background:'#fff',borderRadius:11,padding:'12px 16px',display:'flex',alignItems:'center',justifyContent:'space-between',boxShadow:'0 1px 8px rgba(0,0,0,0.05)',border:'1px solid #F1F5FB',marginBottom:8}}>
            <div style={{display:'flex',alignItems:'center',gap:10}}>
              <div style={{width:34,height:34,borderRadius:8,background:'#EFF6FF',display:'flex',alignItems:'center',justifyContent:'center'}}><FileText size={15} color={BLUE}/></div>
              <div>
  ...
  const renderMessages=()=>(
    <div style={{display:'flex',flexDirection:'column',height:isMobile?'calc(100vh - 200px)':'calc(100vh - 160px)'}}>
      ...
      <div style={{display:'flex',gap:8}}>
        <input value={msgInput} onChange={e=>setMsgInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&sendMsg()} placeholder="Écrire un message..." style={{flex:1,border:'1.5px solid #E2E8F0',borderRadius:10,padding:'10px 13px',fontSize:14,outline:'none',fontFamily:'inherit'}}/>
  ...
  return(
    <div style={{display:'flex',minHeight:'100vh',background:'#F7F8FC',fontFamily:"'DM Sans',sans-serif",flexDirection:isMobile?'column':'row'}}>
      {!isMobile&&<div style={{width:240,background:'#fff',borderRight:'1px solid #F1F5FB',position:'sticky',top:0,height:'100vh',flexShrink:0}}><SidebarContent/></div>}
      {isMobile&&sideOpen&&(
        <div style={{position:'fixed',inset:0,zIndex:100,background:'rgba(0,0,0,0.5)'}} onClick={()=>setSideOpen(false)}>
          <div style={{width:280,height:'100%',background:'#fff'}} onClick={e=>e.stopPropagation()}><SidebarContent/></div>
        </div>
      )}
