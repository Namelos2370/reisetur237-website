...
export default function BlogPreviewSection() {
  ...
  const L = {
    ...
    de:{ badge:'Blog', title:'Neueste Beiträge', sub:'Informationen und Leitfäden für Ihr Mobilitätsprojekt', readMore:'Weiterlesen', allArticles:'Alle Artikel', readTime:'Min.' },
    ...
    <section style={{ padding:'80px 32px', background:'#F7F8FC' }}>
      <div style={{ maxWidth:1160, margin:'0 auto' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:24, marginBottom:36 }}>
          {articles.map(a => (
            <Link key={a.id} to={`/blog/${a.slug||a.id}`} style={{ textDecoration:'none' }}>
              <div style={{ background:'#fff', borderRadius:18, overflow:'hidden', boxShadow:'0 2px 18px rgba(0,0,0,0.07)', border:'1.5px solid #F1F5FB', height:'100%', display:'flex', flexDirection:'column', transition:'all .2s' }}>
                {/* Image */}
                <div style={{ height:200, overflow:'hidden', position:'relative' }}>
                  {a.image
                    ? <img src={a.image} alt={getTitle(a)} style={{ width:'100%', height:'100%', objectFit:'cover' }} loading="lazy"/>
                    : <div style={{ height:'100%', background:`linear-gradient(135deg,${NAVY},#1B3E6F)` }}/>
                  }
                ...
                <div style={{ padding:'18px 20px', flex:1, display:'flex', flexDirection:'column' }}>
                  <h3 style={{ fontFamily:"'Playfair Display',serif", fontSize:16, fontWeight:700, color:NAVY, lineHeight:1.4, marginBottom:12, flex:1 }}>
                    {getTitle(a)}
                  </h3>
        ...
        <div style={{ textAlign:'center' }}>
          <Link to="/blog" style={{ display:'inline-flex', alignItems:'center', gap:7, background:NAVY, color:'#fff', textDecoration:'none', borderRadius:11, padding:'12px 26px', fontSize:14, fontWeight:700, boxShadow:'0 2px 14px rgba(0,0,0,0.15)' }}>
